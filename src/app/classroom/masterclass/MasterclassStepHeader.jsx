// src/app/classroom/masterclass/MasterclassStepHeader.jsx
//
// Same markup / active-done-future states as the public flow's StepHeader,
// with three steps instead of four. The public 4-step header is untouched.

import { Check } from "lucide-react";

const steps = [
  { id: 1, label: "ค้นหาชื่อ" },
  { id: 2, label: "เซ็นลายเซ็น" },
  { id: 3, label: "เสร็จสิ้น" },
];

export default function MasterclassStepHeader({ currentStep }) {
  return (
      ...
      {steps.map((step) => {
        ...
        if (isActive)
          return (
            ...
            >
              ...
              <span className="sm:text-xl lg:text-base font-bold">
                {step.label}
              </span>
            </div>
          );
        ...
        return (
          ...
          >
            ...
            <span className="sm:text-lg lg:text-sm text-slate-300">
              {step.label}
            </span>
          </div>
        );
